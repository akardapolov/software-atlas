# Authors

People who shaped how we think about software. Each card covers
biography, key contributions, works, and influence connections.

✅ = card complete

## By Era

### Foundations (1930s–1960s)

| Author | Years | Known for | Page |
|--------|-------|-----------|------|
| Kurt Gödel | 1906–1978 | Incompleteness theorems | [→](kurt-godel.md) ✅ |
| Alan Turing | 1912–1954 | Turing machine, Church-Turing thesis | [→](alan-turing.md) ✅ |
| Alonzo Church | 1903–1995 | Lambda calculus | [→](alonzo-church.md) ✅ |
| Kristen Nygaard | 1926–2002 | Simula, OOP | [→](kristen-nygaard.md) ✅ |
| John McCarthy | 1927–2011 | Lisp, AI | [→](john-mccarthy.md) ✅ |
| Ivan Sutherland | 1938– | Sketchpad, master/instance | [→](ivan-sutherland.md) ✅ |
| Edsger Dijkstra | 1930–2002 | Structured programming | [→](edsger-dijkstra.md) ✅ |
| Seymour Papert | 1928–2016 | Logo, constructionism | [→](seymour-papert.md) ✅ |
| Christopher Alexander | 1936–2022 | Pattern language | [→](christopher-alexander.md) ✅ |
| Ole-Johan Dahl | 1931–2002 | Simula, OOP | [→](ole-johan-dahl.md) ✅ |

### Crystallisation (1970s–1980s)

| Author | Years | Known for | Page |
|--------|-------|-----------|------|
| John Backus | 1924–2007 | Fortran, FP manifesto | [→](john-backus.md) ✅ |
| Fred Brooks | 1931–2022 | MMM, No Silver Bullet | [→](fred-brooks.md) ✅ |
| Robin Milner | 1934–2010 | ML, type inference | [→](robin-milner.md) ✅ |
| Tony Hoare | 1934– | CSP, null ref, quicksort | [→](tony-hoare.md) ✅ |
| Barbara Liskov | 1939– | CLU, ADT, LSP | [→](barbara-liskov.md) ✅ |
| Alan Kay | 1940– | Smalltalk, OOP, messaging | [→](alan-kay.md) ✅ |
| David Parnas | 1941– | Information hiding | [→](david-parnas.md) ✅ |
| Leslie Lamport | 1941– | Logical clocks, Paxos | [→](leslie-lamport.md) ✅ |
| Jim Gray | 1944–2007 | Transactions, ACID | [→](jim-gray.md) ✅ |
| Edgar F. Codd | 1923–2003 | Relational model | [→](edgar-codd.md) ✅ |
| Michael Stonebraker | 1943– | PostgreSQL, Ingres, NewSQL | [→](michael-stonebraker.md) ✅ |
| Bertrand Meyer | 1950– | Eiffel, DbC, CQS | [→](bertrand-meyer.md) ✅ |
| John Hughes | 1958– | Why FP Matters, QuickCheck | [→](john-hughes.md) ✅ |

### Modern Practice (1990s–present)

| Author | Years | Known for | Page |
|--------|-------|-----------|------|
| Joe Armstrong | 1950–2019 | Erlang, let-it-crash | [→](joe-armstrong.md) ✅ |
| Robert C. Martin | 1952– | SOLID, Clean Code | [→](robert-c-martin.md) ✅ |
| Alistair Cockburn | 1953– | Hexagonal architecture | [→](alistair-cockburn.md) ✅ |
| Kent Beck | 1961– | XP, TDD | [→](kent-beck.md) ✅ |
| Rich Hickey | 1961– | Clojure, Simple≠Easy | [→](rich-hickey.md) ✅ |
| Martin Fowler | 1963– | Refactoring, EAA | [→](martin-fowler.md) ✅ |
| Eric Evans | — | DDD | [→](eric-evans.md) ✅ |
| Gary Bernhardt | — | FC/IS, Boundaries | [→](gary-bernhardt.md) ✅ |
| Martin Kleppmann | — | DDIA | [→](martin-kleppmann.md) ✅ |
| Scott Wlaschin | — | Domain Modeling FP | [→](scott-wlaschin.md) ✅ |
| Ward Cunningham | 1949– | CRC Cards, Technical Debt | [→](ward-cunningham.md) ✅ |
| Sandi Metz | — | POODR, Practical OOP | [→](sandi-metz.md) ✅ |
| Alexander Zhidkov | — | Ergonomic Approach, Structural Design | [→](alexander-zhidkov.md) ✅ |

## Language Creators

| Author | Years | Known for | Page |
|--------|-------|-----------|------|
| Bjarne Stroustrup | 1950– | C++ | [→](bjarne-stroustrup.md) ✅ |
| Brad Cox | 1944–2021 | Objective-C | [→](brad-cox.md) ✅ |
| James Gosling | 1955– | Java | [→](james-gosling.md) ✅ |
| Guido van Rossum | 1956– | Python | [→](guido-van-rossum.md) ✅ |
| Yukihiro Matsumoto | 1965– | Ruby | [→](yukihiro-matsumoto.md) ✅ |

## Methodologists & UML

| Author | Years | Known for | Page |
|--------|-------|-----------|------|
| Grady Booch | 1955– | Booch Method, UML | [→](grady-booch.md) ✅ |
| Ivar Jacobson | 1939– | Use Cases, OOSE | [→](ivar-jacobson.md) ✅ |
| James Rumbaugh | 1947– | OMT, UML | [→](james-rumbaugh.md) ✅ |
| Rebecca Wirfs-Brock | ~1953– | RDD, CRC Cards | [→](rebecca-wirfs-brock.md) ✅ |

## Gang of Four (Design Patterns)

| Author | Years | Known for | Page |
|--------|-------|-----------|------|
| Erich Gamma | 1961– | GoF, JUnit | [→](erich-gamma.md) ✅ |
| Ralph Johnson | 1955– | GoF, Frameworks | [→](ralph-johnson.md) ✅ |
| Richard Helm | 1960–1999 | GoF, Pattern Hatching | [→](richard-helm.md) ✅ |
| John Vlissides | 1961–2005 | GoF, Pattern Hatching | [→](john-vlissides.md) ✅ |

## Summary

**All 52 author cards complete.** ✅

## Influence Web

The authors above are connected by chains of influence:

```
Hilbert → Gödel → Church → McCarthy → Hickey
Hilbert → Gödel → Church → Milner → Haskell → Wlaschin
Hilbert → Gödel → Church → Backus → Hughes → Hickey
Gödel → Turing → von Neumann (stored-program computers)
Dijkstra → Parnas → Brooks
Dijkstra → Hoare → Armstrong
Dahl/Nygaard → Kay → GoF → Fowler
Dahl/Nygaard → Kay → Goldberg/Ingalls
Dahl/Nygaard → Stroustrup → Java/C++
Liskov → Martin (SOLID)
Liskov → Cardelli → Typed OOP
Meyer → Martin (SOLID)
Parnas → Martin (SOLID)
Parnas → Wirfs-Brock → Metz
Hoare → Meyer (DbC ← Hoare Logic)
Hoare → Liskov → ADT
Codd → Stonebraker → PostgreSQL/NewSQL
Codd → Gray → Kleppmann
Lamport → Gray → Kleppmann
Beck → Fowler → industry
Beck → Cunningham → Wirfs-Brock → Metz
Beck → Gamma → GoF
Evans → Wlaschin (DDD + FP)
Cockburn → Evans → Newman
Cunningham → Technical Debt → Fowler
Hickey → Bernhardt
Kay → Cox → Objective-C
Kay → Matsumoto → Ruby
Kay → Van Rossum → Python
Alexander → Beck/Cunningham → GoF
Sutherland → Kay → OOP
Papert → Kay → Education
Constantine → Zhidkov
Evans → Zhidkov
Hickey → Zhidkov
```

See [Ideas Evolution Map](../maps/ideas-evolution.md) for the full visual diagram.
